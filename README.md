...
After dropping its email storage limit from "unlimited" to 1TB in 2013, Yahoo announced at the end of 2025 that it would be further dropping the limit to 15GB for UK and EU customers from May 2026 onwards, blocking accounts over that limit, with no options offered to users other than deleting emails and attachments or paying for additional storage (as Yahoo has a 12-month rule on dormant accounts). This move caught many legacy users off-guard after decades of rather generous storage allowances, and sent many scrambling to downsize personal or professional mailboxes with histories starting in the early 2000s (and which amounted to dozens of GBs and 10s of thousands of emails for many), rushing against the deadline to recover inbox & outbox functionalities (whilst very often moving to a new email account elsewhere, like yours truly). This **Mailbreak** repo was built against this background with tremendous amounts of user frustration and caffeine to reclaim complete ownership of a (rather ginormous) Yahoo email history through a secure, hybrid and cost effective archiving model. By shifting historical mail off Yahoo's proprietary platform onto local infrastructure (i.e. your -hopefully trustworthy- desktop or laptop hard drive), backed by optional cost-effective storage in S3 Glacier (just in case your trust in your hardware was misplaced), this flexible retrieval tool offers privacy and offline query performance to liberate your historical email archive from corporate hostage-taking.
...
* **User Interface**: Streamlit for a user-friendly, browser-based dashboard with flexible search filters.
* **Local Deployment**: using the native email application for Windows, MacOS or Linux to access search results from the Streamlit UI.
